...
Q_age_group_percentage <- flu_linelist %>% 
...
yaounde_age %>%
  mutate(age_group = case_when(age_years < 18 ~ "child", TRUE ~ "Not child"))
```
...
yaounde_educ %>%
  ...
  mutate(highest_educ_recode = case_when( highest_education %in% c("University", "Doctorate") ~ "Post-sceondary" ))
```
...
yaounde_BMI <-
  ...
  mutate(BMI = case_when(BMI < 18.5 ~ "under weight",
                         BMI >= 10.5 & BMI < 24.9 ~ "normal weight",
...
Q_adolescent_grouping <- 
  flu_linelist %>% 
  mutate(adolocents = case_when(age >= 10 & age <= 19 ~ "Yes",
                                TRUE ~ "No"))
Q_adolescent_grouping
```
...
```{r}
yaounde_age_sex %>%
  mutate(recruit = case_when(  sex == "Male" & age_years >= 20 & age_years <= 29 ~ "Recruit to male study",
...
Q_age_province_grouping <- 
  flu_linelist %>% 
  mutate(recruit = case_when( province == "Jiangsu" & age >= 30 & age <= 59 ~ "Recruit to Jiangsu",
                              province == "Zhejiang" & age >= 30 & age <=59 ~ "Recruit to Zhejiang",
                              TRUE ~ "Do not recruit"))
...
yaounde_symptoms_slice %>%
  ...
  mutate(covid_status = 
           case_when( symp_anosmia_or_ageusia == "Yes" ~ "Probable COVID",
...
Q_priority_groups <- 
  flu_linelist %>% 
  mutate(follow_up_priority = 
           case_when(age < 18 ~ "Highest priority",
                     gender == "f"  ~ "high priority",
                     gender == "m" ~ "no priority")
  ) %>% 
  select(age, gender, follow_up_priority)
Q_priority_groups
```
...
yaounde_educ %>% 
  mutate(highest_education = if_else(
    highest_education %in% c("University","Doctorate"),
    "Post-secondary", highest_education
  ))
...
Q_age_group_if_else <- 
  ...
  mutate(age_group = if_else(
    #write the condition
    age < 50 ,
    #outcome (just like ternary operator in JS)
