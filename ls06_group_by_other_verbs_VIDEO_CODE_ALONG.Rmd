...
```{r}
yao_sex_weight %>% 
  mutate( ranks = rank(desc(weight_kg)))
```
...
Q_rank_grip_strength <- 
  ...
  select(age_group, grip_strength_rank, grip_strength_kg)
Q_rank_grip_strength
```
...
![Fig: filter() and its use combined with group_by().](images/custom_dplyr_groupby_filter.png){width="400"}
