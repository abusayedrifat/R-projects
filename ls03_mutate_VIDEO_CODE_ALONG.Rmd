...
```{r}
yao <-  yao %>%
  select(weight_kg,  height_cm, everything()) %>% 
  # first obtain the height in meters  
...
Q_ASM_calculation <- 
  sarcopenia %>% 
  select(weight_kg, height_meters, sex_male_1_female_0, age) %>% 
...
yao %>% 
  select(bmi) %>% 
  mutate(integer_bmi =  as.integer(bmi)) %>% 
  mutate(rounded_bmi = round(bmi))
```
...
mutate(airquality, Temp = 5/9*(Temp-32))
```
