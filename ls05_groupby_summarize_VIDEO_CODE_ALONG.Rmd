...
yao %>%
  ...
  summarise(total_days_missed = mean(n_days_miss_work, na.rm = TRUE))
```
...
Q_weight_by_smoking_status <- 
  yao %>% 
  group_by(is_smoker) %>% 
  summarize(weight_mean = mean(weight_kg))
Q_weight_by_smoking_status
```
...
Q_min_max_height_by_sex <- 
  yao %>% 
  group_by(sex) %>% 
  summarize(min_height_cm = min(height_cm, na.rm = TRUE),
            max_height_cm = max(height_cm, na.rm = TRUE))
...
Q_sum_bedridden_days <- 
  yao %>% 
  group_by(sex) %>% 
  summarize(total_bedridden_days = sum(n_bedridden_days, na.rm = TRUE))
Q_sum_bedridden_days
```
...
Q_bedridden_by_age_sex_iggresult <- 
  ...
  summarize(mean_n_bedridden_days = mean(n_bedridden_days, na.rm = TRUE))
Q_bedridden_by_age_sex_iggresult
```
...
##You are getting NaN (Not a Number) because you are calculating a mean() on groups where every single value in that group is NA.
...
yao %>% 
  ...
  select(treatment_combinations, neighborhood)
```
...
yao %>% 
  ...
  summarize(no_treatments = sum(is.na(treatment_combinations))) 
```
...
count(sex, occupation, name = "count")
...
yao %>% 
  ...
  summarise(count = mean(age, na.rm = TRUE))
```
