...
pacman::p_load(rlang, tidyverse, knitr, here, gt)
...
- Select the 16th and 22nd columns (in that order) in the `yaounde` data frame.
...
```{r, render = reactable_5_rows}
# Bring columns that start with "result" to the front of the data frame
yaounde %>% select(ends_with("result"), everything())
```

::: practice
- Select all columns in the `yaounde` data frame that start with "is".
...
- Move the columns that start with "is" to the beginning of the `yaounde` data frame.
...
```{r}
yao2 <- yaounde |> select(symptoms:sequelae)
yao2
```
...
```{r render=reactable_5_rows}
yao3 <- yaounde |>
  select(!highest_education:consultation)
  yao3
```
...
it finds those variables which start with specific string(provided by user)

### 2. `ends_with()`

it finds those variable with ends with specific string(provided by user)

#### example

```{r re}
q10 <- yao %>% select(starts_with("is_")) 
q11 <-  yao %>% select(ends_with("_result")) 
q10
q11
```
...
it finds those variable contain specific string(provided by user)

```{r, render = reactable_10_rows}
q7 <- yaounde %>%  select(contains("_drug_"))
q7
```

### 4. `everything()`

it puts all variable after the selected variables

```{r, render = reactable_5_rows}
q8 <-  yao %>% select(occupation  ,everything())
q8
```

and further we can make it complex . here given below:

```{r, render = reactable_5_rows}
q8 <-  yao %>% select(occupation, ends_with("_result"),everything())
q8
```
...
yao <-  yaounde %>% 
  select( age, sex, weight_kg, highest_education,neighborhood, occupation, is_smoker, is_pregnant, igg_result, igm_result)

yao

```

#### 1. if we want to keep only male records
...
1.  all of them i already know
2.  **A%in%B –\> it means A is an element of B**

```{r, render = reactable_5_rows}

q23 <- yao %>% filter( sex != "Male") 
q23  

q24 <- yao %>% filter( age < 6) 
q24

q25 <- yao %>% filter( age >= 70) 
q25 


##keep repondents whose highest education is "Primary" or "Secondary" 

q26 <- yao %>%  filter(highest_education %in% c("Primary", "Secondary"))
q26 
```
...
```{r}
## drop respondents, who are children or wiight less than 30 kg

q27 <- yao %>% filter(!(age<18 | weight_kg<30))
q27

## drop respondents whose highest education is "Primary" or "Secondary"
q28 <- yao %>% filter(!(highest_education %in% c("Primary", "Secondary"))) 
q28
```

### `NA` values

we can't use usual logical function or method to avoid `NA` values. In that case we use `is.na()` function to determine which values are empty.
...
```{r, render= reactable_7_rows}

q21 <- yao %>% filter(is_pregnant == "Yes")
q21
```
...
q22 <- yao %>%  filter(sex == "Female")
q22
```

```{r}
## keep only respondents who were children (under 18)

qq3 <- yao %>% filter( age < 18)
qq3

## keep only thosne respondents who live in "Tsinga" or "Messa"
qq4 <-  yao %>%  filter(c("Tsinga","Messa"))
qq4
```

```{r}
## keep man who tasted IgG positive

qq5 <-  yao %>% filter( sex == "Male" &  igg_result == "Positive") %>% 
  select(sex, igg_result, everything())
qq5

## include just children and those whoes highest education is primary school

qq6 <-yao %>%  filter( age<18 & highest_education=="Primary") %>% 
  select(age, highest_education , everything())
qq6
```
